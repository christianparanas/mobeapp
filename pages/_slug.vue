        ...
        <div class="movie_detail">
          ...
          <div class="mini-infos">
            <div class="movie__release-date">
              ...
              <div class="">{{ contentData.release_date }}</div>
            </div>
            <div class="movie__release-date">
              <div class="">Status</div>
              <div class="">{{ contentData.status }}</div>
            </div>
            <div class="movie__release-date">
              <div class="">Rating</div>
              <div class="">{{ contentData.vote_average }}</div>
            </div>
            <div class="movie__release-date">
              <div class="">Runtime</div>
              <div class="">{{ contentData.runtime }}min</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="slug-body">
        ...
        <div class="simillar__movies" v-if="haveSimillar">
          ...
          <div class="simillarMoviesWrapper">
            <ContentPoster
              v-for="simillarmovie in simillarMovies.results"
              :content="simillarmovie"
              :key="simillarmovie.id"
            />
          </div>
        </div>
      </div>
...
export default {
  ...
  async asyncData({ params, $axios }) {
    let contentData = await $axios.$get(
      `https://api.themoviedb.org/3/movie/${parseInt(
        params.slug
      )}?api_key=a807f0095433ac989503323b5b0bc933&language=en-US`
    );
    ...
    let casts = await $axios.$get(
      `https://api.themoviedb.org/3/movie/${parseInt(
        params.slug
      )}/credits?api_key=a807f0095433ac989503323b5b0bc933&language=en-US`
    );

    let movieTrailer = await $axios.$get(
      `https://api.themoviedb.org/3/movie/${parseInt(
        params.slug
      )}/videos?api_key=a807f0095433ac989503323b5b0bc933&language=en-US`
    );

    let simillarMovies = await $axios.$get(
      `https://api.themoviedb.org/3/movie/${parseInt(
        params.slug
      )}/similar?api_key=a807f0095433ac989503323b5b0bc933&language=en-US&page=1`
    );
    ...
    if (!movieTrailer.results.length == 0) {
      ...
      videoId =
        getId(`http://www.youtube.com/watch?v=${trailer[0].key}`) ||
        getId(`http://www.youtube.com/watch?v=${trailer[0].key}`);
